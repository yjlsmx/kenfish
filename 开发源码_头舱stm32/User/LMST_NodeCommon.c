...
void YJ_InitSystem()
{
	...
	if(YJ_GetNodeType() != LM_LOCAL_CAN_ADDR && (YJ_GetNodeType() != LM_LOCAL_CAN_ADDR - 1))
	{
		YJ_ReBoot();
		while(1);
	}
...
int32_t YJ_PackedApl(char *buf, int32_t type,char *data, int32_t datalen)
{
	  ...
	  int32_t 	addr  = LM_LOCAL_CAN_ADDR;
		...
		if(addr == LMST_HEAD_LOCAL_CTRL_NODE)
			addr = LMST_HEAD_NODE;
