...
void SaveParameter(Config *cfg)
{
	...
	ConfigSave.ServoOffset[0] = cfg->ServoOffset[0];
	ConfigSave.ServoOffset[1] = cfg->ServoOffset[1];
	ConfigSave.ServoOffset[2] = cfg->ServoOffset[2];
	ConfigSave.LServoOffset_sm 	= cfg->LServoOffset_sm;
	ConfigSave.RServoOffset_sm 	= cfg->RServoOffset_sm;
	ConfigSave.Version = VERSION;
	FlashWriteWords(ConfigAddress,(u32 *)&ConfigSave,6);
	/*
	PID *PIDRead;
	PIDSave=&Roll;
...
void ReadParameter(Config *cfg)
{
	Config ConfigRead;
	
	FlashReadWords(ConfigAddress,(u32 *)&ConfigRead,6);
	if(ConfigRead.Version == VERSION)
	{
		cfg->ServoOffset[0] 	= ConfigRead.ServoOffset[0];
		cfg->ServoOffset[1] 	= ConfigRead.ServoOffset[1];
		cfg->ServoOffset[2] 	= ConfigRead.ServoOffset[2];
		cfg->LServoOffset_sm 	= ConfigRead.LServoOffset_sm;
		cfg->RServoOffset_sm 	= ConfigRead.RServoOffset_sm;
	}
	else
	{
...
{
		FlashWriteWords(YJ_NODE_TYPE_FLASH_ADDR,(int32_t *)&type, 1);
}
...
{
		int32_t type;
		FlashReadWords(YJ_NODE_TYPE_FLASH_ADDR, (u32 *)&type, 1);
		return type;
}
